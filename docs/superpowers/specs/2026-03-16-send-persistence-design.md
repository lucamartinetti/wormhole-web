...
Decouple the wormhole protocol exchange from upload timing so that uploaded files remain available until a receiver connects, rather than expiring immediately.
...
### Core change: start PAKE early

Split the send flow into two phases:

**Phase 1 — Session creation (`POST /send/new` or `PUT /send` redirect):**
- Create wormhole, allocate code
- Start PAKE key exchange in the background (fire-and-forget Deferred)
- Store the key exchange Deferred in the session object
- Return the code to the caller immediately

**Phase 2 — Upload (`PUT /send/<code>`):**
- Read filename and filesize from request headers
- Wait for Phase 1's key exchange to complete (yield the stored Deferred)
- Send transit hints and file offer
...
```
start_key_exchange(wormhole, reactor, timeout) -> Deferred
    Starts PAKE key exchange. Returns a Deferred that fires when
    key exchange completes (get_unverified_key + get_verifier).
    Does NOT send transit hints, offer, or establish transit.
    This Deferred is stored in the Session object.
...
complete_send(wormhole, key_exchange_d, filename, filesize, reactor, timeout) -> Connection
    ...
    Sends transit hints and file offer.
    Waits for receiver's transit hints and file_ack.
    Establishes and returns the transit connection.
```

### Session changes

Add a `key_exchange_d` field to `Session` to hold the key exchange Deferred from Phase 1.
...
| Receiver connects after upload | Phase 2 writes "waiting for receiver...", blocks on PAKE/offer/ack. Sender's curl hangs. When receiver connects, data flows from disk buffer. |
| Receiver never connects | Phase 2 blocks until `--transfer-timeout` expires, then returns an error to the sender's curl. |
| Sender never uploads (no PUT) | Session TTL (60s) expires, wormhole is closed and cleaned up. |
